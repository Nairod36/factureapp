...
const { createInvoiceOnChain, getInvoiceStatus, getMerchantInvoices, getTotalInvoiceCount, checkConnection } = require('../../contract/web3_service');
...
// Route pour créer une facture sur le smart contract
router.post('/contract/create', async (req, res) => {
  const { clientAddress, amount } = req.body;
  try {
    const result = await createInvoiceOnChain(clientAddress, amount);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: 'Erreur création facture blockchain', details: err.message });
  }
});

// Route pour vérifier le statut de paiement on-chain
router.get('/contract/status/:id', async (req, res) => {
  try {
    const result = await getInvoiceStatus(req.params.id);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: 'Erreur statut blockchain', details: err.message });
  }
});
