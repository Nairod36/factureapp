...
// Route pour initier un transfert CCTP (Circle Cross-Chain)
router.post('/cctp', async (req, res) => {
  const { amount, fromChain, toChain, toAddress, partner } = req.body;
  try {
    const result = await createCctpMessage({ amount, fromChain, toChain, toAddress, partner });
